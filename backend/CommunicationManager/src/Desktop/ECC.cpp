...
-add deconstructor for all the structs
...
PublicKey ECC::generatePublicKey(const BigInt<4>& privKey, const ECCurve& curve){
    PublicKey pub;
    pub.Q = scalarMult(curve.G, privKey, curve);
    return pub;
}

Signature ECC::sign(const BigInt<4>& priv, const BigInt<4>& hash, const ECCurve& curve){
    
    Signature sig;

    while(true){
        BigInt<4> k;
        randomScalar(curve.n, k);
        ...
        BigInt<4> kinv = modInv(k, curve.n);
...
bool ECC::verify(const PublicKey& pub, const Signature& sig, const BigInt<4>& hash, const ECCurve& curve){
...
//This function may need to loop less during debugging if there's an issue so the outputs can be readable
ECPoint ECC::scalarMult(const ECPoint& P, const BigInt<4>& k, const ECCurve& curve){
...
void ECC::randomScalar(const BigInt<4>& n, BigInt<4>& out){
    out = 1;
}